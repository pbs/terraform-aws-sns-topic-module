...
```hcl
module "sns-topic" {
    source = "github.com/pbs/terraform-sns-topic-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/sns-topic'
gh -R 'pbs/terraform-sns-topic-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/sns-topic'
rm -f release.tar.gz
```
...
```bash
git remote add -f sns-topic git@github.com:pbs/terraform-sns-topic-module.git
git subtree add --prefix terraform/modules/sns-topic sns-topic main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/sns-topic sns-topic $REF --squash
```
